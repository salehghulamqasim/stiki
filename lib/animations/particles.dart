...
class _ParticlesState extends State<Particles>
  ...
  Particle _createParticle() {
    return Particle(
      x: random.nextDouble() * (canvasSize.width > 0 ? canvasSize.width : 500),
      y:
          random.nextDouble() *
          (canvasSize.height > 0 ? canvasSize.height : 500),
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (particles.isEmpty && canvasSize.width > 0) {
          _initParticles();
        }
...
class ParticlesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final particle in particles) {
      final paint = Paint()
        ..color = color.withValues(alpha: particle.alpha.clamp(0.0, 1.0))
